...
loggingRouter.post("/:id/log", (req, res) => {
  ...
  db.prepare(
    `INSERT INTO habit_logs (habit_id, user_id, date, notes)
     VALUES (?, ?, ?, ?)
     ON CONFLICT(habit_id, date) DO UPDATE SET notes = excluded.notes, completed_at = CURRENT_TIMESTAMP`
  ).run(habitId, userId, date, notes ?? null);
...
loggingRouter.delete("/:id/log", (req, res) => {
  ...
  const habitId = Number(req.params.id);
  const date = todayDate();
  db.prepare("DELETE FROM habit_logs WHERE habit_id = ? AND user_id = ? AND date = ?").run(
    habitId,
    userId,
    date
  );
